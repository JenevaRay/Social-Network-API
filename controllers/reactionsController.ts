...
async function getReactions(req, res) {
    try {
        const thoughts = await Thought.find({ reactions: { $exists: true, $ne: [] } })
        res.json(thoughts)
    } catch (err) {
...
async function getReaction(req, res) {
    try {
        const thought = await Thought.findOne({ reactions: { $exists: true, $ne: [] } }, { reactions: { $elemMatch: { _id: req.params.reactionId } } })
        res.json(thought)
    } catch (err) {
        res.status(500).json(err)
    }
...
async function updateReaction(req, res) {
    try {
        ...
        if (thoughtByReaction && user && reactionBody) {
            for (let reaction in thoughtByReaction.reactions) {
                if (thoughtByReaction.reactions[reaction]._id == req.params.reactionId) {
...
export { getReactions, createReaction, getReaction, updateReaction, deleteReaction }